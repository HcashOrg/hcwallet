...
func lastUsedAddressIndexUpgrade(tx walletdb.ReadWriteTx, publicPassphrase, privatePassphrase []byte) error {
	if len(privatePassphrase) == 0 {
		return indexUpgradeByPub(tx, publicPassphrase)
	} else {
		return indexUpgradeBytPubAndPri(tx, publicPassphrase, privatePassphrase)
	}
}

func indexUpgradeByPub(tx walletdb.ReadWriteTx, publicPassphrase []byte) error {
	const oldVersion = 1
	const newVersion = 2
	...
	for account := uint32(0); account <= lastAccount; account++ {
		...
		for child := uint32(0); child < hdkeychain.HardenedKeyStart; child++ {
			...
			addr, _ := xpubChild.Address(&chaincfg.MainNetParams, uint8(chainec.ECTypeSecp256k1))
...
func indexUpgradeBytPubAndPri(tx walletdb.ReadWriteTx, publicPassphrase, privatePassphrase []byte) error {
	const oldVersion = 1
	const newVersion = 2
