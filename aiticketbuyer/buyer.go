...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		t.firstStart = false
		log.Debugf("First run for ai ticket buyer")
	...
	if refreshStakeInfo {
		...
		t.stakePoolSize = curStakeInfo.AiPoolSize
		t.stakeLive = curStakeInfo.AiLive
		t.stakeImmature = curStakeInfo.AiImmature
	...
	if err != nil {
		return ps, fmt.Errorf("Failed to calculate ai average ticket "+
			" price amount at height %v: %v", height, err)
	}
	if avgPriceAmt < hcutil.Amount(t.activeNet.MinimumAiStakeDiff) {
		avgPriceAmt = hcutil.Amount(t.activeNet.MinimumAiStakeDiff)
	}

	log.Debugf("Calculated average ai ticket price: %v", avgPriceAmt)
	ps.PriceAverage = avgPriceAmt

	nextStakeDiff, err := t.wallet.AiStakeDifficulty()
	...
	sDiffEsts, err := t.hcdChainSvr.EstimateAiStakeDiff(nil)
	if err == nil {
		...
		log.Tracef("Estimated ai stake diff: (min: %v, expected: %v, max: %v)",
	...
	log.Tracef("Own ai tickets in mempool: %v", inMP)
	...
	oneBlock := uint32(1)
	info, err := t.hcdChainSvr.AiTicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	if len(info.AiFeeInfoBlocks) < 1 {
		return ps, fmt.Errorf("error FeeInfoBlocks bad length")
	}
	ticketPurchasesInLastBlock := int(info.AiFeeInfoBlocks[0].Number)
	log.Tracef("Ai Ticket purchase slots filled in last block: %v", ticketPurchasesInLastBlock)
	...
	maxStake := int(t.activeNet.AiMaxFreshStakePerBlock)
	...
	t.wallet.SetAiTicketFeeIncrement(feeToUse)
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		tixWillRedeem := float64(blocksRemaining) * float64(t.activeNet.AiTicketsPerBlock) * proportionLive
	...
	for i := range hashes {
		log.Infof("Purchased ai ticket %v at stake difficulty %v (%v "+
			"fees per KB used)", hashes[i], nextStakeDiff.ToCoin(),
			feeToUse.ToCoin())
	}
	ps.Purchased = len(hashes)
	if purchaseErr != nil {
		log.Errorf("One or more ai tickets could not be purchased: %v", purchaseErr)
	}
